...
void JetUserData::produce( edm::Event& iEvent, const edm::EventSetup& iSetup) {
	...
	for (size_t i = 0; i< jetColl->size(); i++){
		...
		jetParam.setJetPt((1+jecUncertainty_up)*jetCorrFactor*rawJetP4.pt()).setJetEta(jet.eta()).setRho(*rho);
		float PtResolution = resolution.getResolution(jetParam);
		float JERSF_temp        = res_sf.getScaleFactor(jetParam);
		smearedP4_JEC_up *= get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor*(1+jecUncertainty_up));

		jetParam.setJetPt((1-jecUncertainty_down)*jetCorrFactor*rawJetP4.pt()).setJetEta(jet.eta()).setRho(*rho);
		float PtResolution = resolution.getResolution(jetParam);
		float JERSF_temp        = res_sf.getScaleFactor(jetParam);
		smearedP4_JEC_down *= get_JER_corr(JERSF_temp, isMC, jet, coneSize_, PtResolution, jetCorrFactor*(1-jecUncertainty_down));
